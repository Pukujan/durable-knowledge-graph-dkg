...
PACK_ROOT = REPO_ROOT / "examples" / "packs" / "fossil-agent-engineering"
FIXTURE_PATH = REPO_ROOT / "examples" / "conversation-lineage" / "fossil-agent-engineering-substrate-v1.json"
TRANSCRIPT_PATH = REPO_ROOT / "docs" / "recovery" / "2026-08-16-fossil-handoff-process-transcript.md"
SHARED_URL = "https://chatgpt.com/share/6a820cf1-1060-83ea-9831-82a8ae4c4ddb?ogimg=plain"
...
def _claim_id(node_id: str) -> str:
    return f"clm_fossil_agent_engineering_{node_id.removeprefix('ln_fossil_')}"
...
def _event_base(
    ...
    *,
    ...
    return {
        ...
        "actor": {
            "actor_type": "importer",
            "actor_id": "fossil-agent-engineering-importer-v1",
            "model_id": None,
        ...
        "provenance": {
            ...
            "benchmark_ref": "examples/conversation-lineage/fossil-agent-engineering-substrate-v1.json",
        },
    }
...
def _write_artifact_index(artifact_root: Path) -> None:
    ...
    index.write_text("".join(_canonical(item) + "\n" for item in manifests), encoding="utf-8")


def _capture_shared_page() -> bytes:
    request = Request(SHARED_URL, headers={"User-Agent": "fossil-core-research-ingestion/1.0"})
...
def _already_ingested() -> dict[str, Any] | None:
    ...
    if not event_root.is_dir() or not any(event_root.glob("*/*.json")):
        return None
    report = validate_pack_fixtures([PACK_ROOT], schemas_root=REPO_ROOT / "schemas")
    return {
        "pack_id": PACK_ID,
...
def ingest() -> dict[str, Any]:
    ...
    events = DurableEventStore(PACK_ROOT / "events", REPO_ROOT / "schemas" / "events" / "v1.schema.json")
    ...
    raw_snapshot = source_store.put_snapshot(
        ...
        quality={
            ...
            "reproducibility": 0.8,
            "timeliness": 1.0,
            "notes": "Raw public shared-page HTML captured for provenance; rendered message content remains reconstructed.",
        },
    ...
    transcript_snapshot = source_store.put_snapshot(
        ...
        quality={
            ...
            "reproducibility": 0.8,
            "timeliness": 1.0,
            "notes": "Normalized reconstruction derived from the raw shared-page capture; not a verbatim export.",
        },
    ...
    for edge in fixture["edges"]:
        ...
        relation_id = f"rel_fossil_agent_engineering_{edge['edge_id'].removeprefix('le_fossil_')}"
